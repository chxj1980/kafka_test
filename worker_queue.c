...
static void msg_consume (rd_kafka_message_t *msg, void *opaque)
{
   ...
   {
       if (__sync_add_and_fetch(&finished, 1) == partitions)
           running = 0;
       return;
   }
   else
...
static void* work(void* p) 
{
    ...
    rd_kafka_queue_t *queue = rd_kafka_queue_new(kafka); 
    int partition = (int)p;
    for (int i = 0; i < step; i++)
    {
    if (partition + i >= partitions) break;
    if (rd_kafka_consume_start_queue(topic, partition+i, RD_KAFKA_OFFSET_BEGINNING, queue) == -1)
    {
    ...
    while(running)
    {
        rd_kafka_consume_callback_queue(queue, 1000, msg_consume, 0);
    }
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    int thread_count = 4;    
    step = partitions / thread_count + (partitions % thread_count > 0);
    for (int i = 0; i < thread_count; i++)
    {
        pthread_create(threads + i, 0, work, (void*)(i * step));
    }
    for (int i = 0; i < thread_count; i++)
    {
